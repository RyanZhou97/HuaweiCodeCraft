...
void common_input(char * info[MAX_INFO_NUM]) 
{
	...
	sscanf(info[7+VM_type_num],"%s",predictStartDate);  // get 2015-02-20  no hava 00:00:00
	sscanf(info[8+VM_type_num],"%s",predictEndDate);	  // and so go
	//printf("predictStartDate:%s predictEndDate:%s\n",predictStartDate,predictEndDate);
